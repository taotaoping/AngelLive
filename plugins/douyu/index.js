...
function _douyu_extractRoomIdFromPage(html) {
  ...
  for (const pattern of patterns) {
    const roomId = _lp_firstMatch(text, pattern);
  ...
  for (const cdnItem of cdns) {
    ...
    const qualitys = multirates.map(function (q) {
      return {
        ...
        requestContext: { rate: String(q.rate || 0), qn: String(q.rate || 0) },
        playbackHints: {
          streamFormat: "flv",
    ...
    out.push({
      ...
      displayName: String(cdnItem.name || serverCdn || ""),
      requestContext: { cdn: serverCdn },
      qualitys
    });
  }
  return out;
...
async function _douyu_resolveShare(shareCode) {
  ...
  let fallbackRoomId = "";
  let candidateUrl = "";
  if (_lp_isNumericId(input)) {
    fallbackRoomId = input;
    candidateUrl = `https://www.douyu.com/${input}`;
  }

  if (input.includes("douyu.com") || input.includes("douyu.tv")) {
    candidateUrl = _lp_firstURL(input) || candidateUrl;
    fallbackRoomId = _lp_firstMatch(input, /douyu\.(?:com|tv)\/(\d+)/) || fallbackRoomId;
    fallbackRoomId = _lp_firstMatch(input, /rid=(\d+)/) || fallbackRoomId;
  }
  ...
  if (candidateUrl) {
    ...
    let roomId = _douyu_extractRoomIdFromPage(resp.bodyText || "");
    if (_lp_isNumericId(roomId)) return roomId;

    roomId = _lp_firstMatch(resp.url || "", /(?:douyu\.(?:com|tv)\/|rid=)(\d+)/);
    if (_lp_isNumericId(roomId)) return roomId;
  ...
  if (_lp_isNumericId(fallbackRoomId)) return fallbackRoomId;
...
globalThis.LiveParsePlugin = {
  ...
  async getPlayback(payload) {
    ...
    const requestedRate = Number((payload && (payload.rate !== undefined ? payload.rate : payload.qn)) || 0);
    const requestedCdn = String((payload && payload.cdn) || "").trim();
    return await _douyu_getPlayback(roomId, requestedRate, requestedCdn || null);
  },
  ...
  async refreshPlayback(payload) {
    ...
    const requestedRate = Number(
      qualityContext.rate !== undefined ? qualityContext.rate
        : (qualityContext.qn !== undefined ? qualityContext.qn : (quality.qn || 0))
    );
    const requestedCdn = String(
      cdnContext.cdn || cdnContext.cdnId || cdn.cdn || ""
    ).trim();

    const refreshed = await _douyu_getPlayback(roomId, requestedRate, requestedCdn || null);
    let picked = null;
    for (const item of refreshed) {
      const qualitys = item && item.qualitys ? item.qualitys : [];
      for (const detail of qualitys) {
        if (!picked) picked = detail;
        if (requestedRate > 0 && Number(detail && detail.qn || 0) === requestedRate) {
          picked = detail;
          break;
        }
      }
      if (picked && requestedRate > 0 && Number(picked.qn || 0) === requestedRate) break;
    }
    if (!picked) _douyu_throw("NOT_FOUND", "refreshed playback is empty", { roomId });
